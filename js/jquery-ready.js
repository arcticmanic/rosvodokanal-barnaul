$(document).ready(function () {
    ...
    $('.header .all-sites>a').click(function () {
        $parent = $('.header .all-sites');
        $('.city-list', $parent).slideToggle();
    });

    $(document).click(function (e) {
        if (e.target !== $service_item[0] && !$service_item.has(e.target).length) {
                ...
                $control.on({
                    click: function () {
                        ...
                        if (!$this.hasClass(_activeClass)) {
                            ...
                            var $current = $tabs.filter('[data-tab=' + $this.data('tab') + ']').addClass(_activeClass);
                            $(".hide-block").slideUp();
                    ...
                    mouseenter: function () {
                        var $this = $(this);
                        $tabs.filter('[data-tab=' + $this.data('tab') + ']').addClass(_hoverClass);
                    },
                    mouseleave: function () {
                        var $this = $(this);
                        $tabs.filter('[data-tab=' + $this.data('tab') + ']').removeClass(_hoverClass);
                    }
                });
                $tabs.on({
                    click: function () {
                        ...
                        if (!$this.hasClass(_activeClass)) {
                            ...
                            $control.removeClass(_activeClass).filter('[data-tab=' + $this.data('tab') + ']').addClass(_activeClass);
                            // $tabs.hide().filter('[data-tab=' + $this.data('tab') + ']').show();
                            if (callback instanceof Function) callback($this);
                        }
                        else {
                    ...
                    mouseenter: function () {
                        var $this = $(this);
                        $control.filter('[data-tab=' + $this.data('tab') + ']').addClass(_hoverClass);
                    },
                    mouseleave: function () {
                        var $this = $(this);
                        $control.filter('[data-tab=' + $this.data('tab') + ']').removeClass(_hoverClass);
                    }
                })
            }
        }
    ...
    $(".incr-input__div").append('<div class="inc input-button"><img src="./img/arr-r-br.png"></div><div class="dec input-button"><img src="./img/arr-l-br.png"></div>');
    ...
    $('.jumbotron__slider').on('init reInit', function (event, slick, currentSlide, nextSlide) {
        //currentSlide is undefined on init -- set it to 0 in this case (currentSlide is 0 based)
        var i = (nextSlide ? nextSlide : 0) + 1;
    ...
    $(window).on(" load ", function () {
        ...
        if (_w <= 991) {
            ...
            $(".header-phone").detach().prependTo('.mobile-menu .menu-content');
            $(".header-btn").detach().appendTo('.mobile-menu .wrapper .menu-content ');

            $(".search-menu").detach().appendTo('.mobile-menu  .wrapper  .menu-content');

            $(".all-sites").detach().appendTo('.mobile-menu .wrapper  .menu-content');
            $(".header-soc-list").detach().appendTo('.mobile-menu  .wrapper .menu-content');
